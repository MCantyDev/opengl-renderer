...
struct Light 
{
	vec3 direction; // Direction Light is Shining
...
void main()
{
	...
	if (useTexture)
	{
		ambient = light.ambient * texture(material.diffuse, textureCoords).rgb; // Ambient Lighting based on the diffuse texture
		diffuse = light.diffuse * diff * texture(material.diffuse, textureCoords).rgb; // Diffuse texture using the diffuse map
		specular = light.specular * spec * texture(material.specular, textureCoords).rgb; // Specular based on the specular map
