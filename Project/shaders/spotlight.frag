...
void main()
{
	...
	// Theta is going to be a value from 0.0 -> 1.0 (Where 1.0 is aligned and 0.0 is perpendicular)
	float theta = dot(lightDir, normalize(-light.direction));

	// Using a > instead of < due to the way Cosine values work
	// The closer a value is to 1 the smaller the angle is and for that reason we invert the check
	// The smaller the angle, the larger the cosine value (1.0 for 0 degrees, 0.0 for 90 degrees)
